			...
			try {
				...
				for (FileItem item : items) {
					...
					if (item.isFormField()) {
						...
						// 是上传组件.
						filename = item.getName(); // 获取上传文件名称
						oitem = item;
						// 得到真实名称
						if (!filename.equals("")) {
				...
				Dish orip = service.findById(p.getId());
				System.out.println("orip.getImgurl()========" + orip.getImgurl());
				System.out.println("p.getImgurl()========" + p.getImgurl());
				if (!orip.getImgurl().equals(p.getImgurl())) {
					...
					IOUtils.copy(oitem.getInputStream(), new FileOutputStream(dest));
					...
					// 封装imgurl
					p.setImgurl(new String("/upload" + uuidDir + "/" + uuidname));

				}
				System.out.println("*****************" + p);
				service.updateDish(p);
